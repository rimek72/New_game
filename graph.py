...
WIDTH = 30   # width of the axis display canvas
class disp:
    """
    ...
    def __init__(self, parent, width=400., height=100.,color = 'ivory'):
        ...
        f.pack(side=TOP)
        self.yaxis = Canvas(f, width = WIDTH, height = height, bg = self.bgcolor)
        ...
        self.xaxis = Canvas(f1, width = width, height = WIDTH, bg = self.bgcolor)
        self.xaxis.pack(side = LEFT, anchor = N, padx = self.border)
        self.canvas.create_rectangle ([(1,1),(width,height)], outline = self.bordcol)
        self.canvas.pack()
        Canvas(f, width = WIDTH, height = height, bg = self.bgcolor).pack(side=LEFT) # spacer only
    ...
    def mark_axes(self, xlab='milli seconds', ylab='mV', numchans=1):
        ...
        self.xtext.append(self.xaxis.create_text(int(self.SCX/2) \
                                                 , WIDTH, text = xlab, anchor=S, fill = self.textcolor))
        ...
        for y in range(0,6):
            ...
            if y == 0: adjust = 6
            if y == 5: adjust = -5
            t = self.yaxis.create_text(WIDTH, int(y * dy) + adjust, \
